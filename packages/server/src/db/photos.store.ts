...
export function createPhoto(
    photo: Omit<StoredPhoto, 'createdAt' | 'bundleId' | 'side' | 'isPreferred'>,
    thumbnail?: Buffer | null,
): StoredPhoto {
    const db = getDb();
    db.prepare(
        'INSERT INTO photos (id, content_hash, file_name, mime_type, file_size, taken_at, thumbnail) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ).run(photo.id, photo.contentHash, photo.fileName, photo.mimeType, photo.fileSize, photo.takenAt, thumbnail ?? null);
...
export function getPhotoThumbnail(photoId: string): Buffer | null {
    const row = getDb()
        ...
        .get(photoId) as { thumbnail: Buffer | null } | undefined;
...
export interface PhotoManifestEntry {
    ...
    /** Base64-encoded JPEG thumbnail, generated by the scanner and stored for recovery purposes. */
    thumbnail?: string;
}
