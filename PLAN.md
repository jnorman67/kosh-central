...
- **Admin: resolve disputed and suggested subjects.** When a viewer clicks the flag icon to dispute an identity tag on a photo, there needs to be an admin workflow to adjudicate it. Add an admin view (likely under the existing persons/subjects admin area) that lists open disputes and suggestions — showing the photo, the current subject assignment, and the dispute comment. The admin can confirm the identity (dismiss the dispute), correct it (re-assign to a different person or remove the tag), or mark it as unresolvable. When resolved, the dispute comment should be marked as resolved (add a `resolved_at` + `resolved_by` to the relevant table) and the flag indicator on the photo should clear.
- **scan-local.ts can skip "archive", "archived", and "ignore" folders**
- **Stale SHA references after in-place file overwrites.** When a photo is rotated or otherwise edited in place on OneDrive, the file bytes change but the filename stays the same. The DB retains the old hash as the preferred photo; a subsequent re-scan adds the new hash to the same bundle but leaves it non-preferred. The app continues to serve the file at the OneDrive location (so users see the rotated version regardless), but the DB's preferred-photo pointer is now pointing to bytes that no longer exist. Fix: during manifest sync, detect when a known location's (folderName, fileName) pair now maps to a different contentHash, retire the old photo as non-preferred, and auto-promote the new hash. The stored bundle thumbnail (a pre-rotation snapshot) remains intact as the recovery anchor.
- **Write metadata back to OneDrive (reverse manifest).** To ensure curator knowledge survives through the ages — outlasting the database, the app, and any infrastructure — export what we know about each bundle (thumbnail, file names, subjects, comments, other metadata) as sidecar JSON files written back into each OneDrive folder alongside the photos. Likely one `kosh-metadata.json` per folder (paralleling the per-folder structure of `scan-local.ts`) rather than one file per bundle. Requires upgrading the Graph API permission scope from `Files.Read.All` to `Files.ReadWrite.All` in the Azure app registration and adjusting the MSAL scopes. Design questions to settle: write-on-save vs. on-demand vs. scheduled, and how conflicts are handled if the file is manually edited externally.
- **Email notifications for @mentions.** When a user is @mentioned in a comment, send them an email notification — at most once per day, even if they were mentioned in multiple conversations that day. The daily digest email should include: who mentioned them and in which photo(s)/album(s), a snippet of each comment, and a direct link to each conversation (i.e. the app URL deep-linking to the photo). Implementation notes: add a `mention_notifications` table (or a flag + `last_notified_at` column on users) to track when the last email was sent per user; a background job (or a trigger on comment insert) checks for unnotified mentions and sends a digest if the last notification was more than 24 hours ago. Sending via a transactional email provider (e.g. SendGrid, Resend) with an `EMAIL_API_KEY` env var. Prerequisite: the user↔person link (see "Link users to persons" above) must be in place so @mentions of a person can be routed to their registered email address.
