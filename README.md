...
**Duplicate relations.** Two kinds of duplicates are tracked separately. *Scanner-detected duplicates* share the same SHA-256 hash — the same bits in more than one place. These can eventually be written into the manifest by the scanner. *User-asserted duplicates* represent the same physical object captured independently (different scans, different family members' albums) and therefore have different hashes; only a human can recognise them. User-asserted `duplicate-of` relations are stored in the database and are never overwritten by a manifest re-import — they are user-curated knowledge, exactly what the site exists to preserve.
